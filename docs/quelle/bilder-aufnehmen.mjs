...
// ---------------------------------------------------------------- Portal
{
  ...
  await p.waitForTimeout(800)
  await p.getByPlaceholder(/Lagemeldung/).first().fill('Sanität ist eingetroffen, Zugang über den Haupteingang freihalten.')
  await p.getByRole('button', { name: 'Senden' }).first().click()
  await schuss(p, 'web-17-alarmzentrale-aktiv', 6000)
...
// ---------------------------------------------------------------- App
{
  ...
  await p.waitForTimeout(600)
  await p.getByPlaceholder(/Rückkehr/).fill('Rückkehr ab 10:30 über den Haupteingang.')
  await p.getByRole('button', { name: 'Entwarnung senden' }).click()
  await p.waitForTimeout(800)
  await abmelden(p)
  await anmelden(p, 'lea.weber@sonnenberg-baar.ch')
  await p.goto(BASIS + '/#/app', { waitUntil: 'networkidle' })
  ...
  // Auslösen: danach Status mit «Fehlalarm melden»
  await halten(p, p.getByText(/alarmieren \(/).last())
  await p.waitForTimeout(6000)
  await p.getByText('Alarm ausgelöst').first().scrollIntoViewIfNeeded()
  await schuss(p, 'app-05c-ausgeloest', 800)
